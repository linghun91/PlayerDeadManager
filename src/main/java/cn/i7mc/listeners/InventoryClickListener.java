...
public class InventoryClickListener implements Listener {
     ...
     * @return 是否为传送GUI
     */
    private boolean isTeleportGUI(@NotNull InventoryClickEvent event) {
        // 通过GUI标题判断（从配置文件获取）
     ...
     * @return 是否为墓碑物品GUI
     */
    private boolean isTombstoneItemsGUI(@NotNull InventoryClickEvent event) {
        // 通过GUI标题判断（从配置文件获取）
        Map<String, String> placeholders = messageManager.createPlaceholders();
     ...
     * @return 是否有权限
     */
    private boolean hasGUIPermission(@NotNull Player player, @NotNull String guiType) {
        String permission = "playerdeadmanager.gui." + guiType.toLowerCase();
        return player.hasPermission(permission) || player.hasPermission("playerdeadmanager.gui.*");
    }
     ...
     * @param guiType GUI类型
     */
    private void sendNoPermissionMessage(@NotNull Player player, @NotNull String guiType) {
        Map<String, String> placeholders = messageManager.createPlaceholders();
        messageManager.addPlayerPlaceholders(placeholders, player);
        placeholders.put("gui_type", guiType);
        messageManager.sendMessage(player, "permission.no-gui-specific", placeholders);
    }
